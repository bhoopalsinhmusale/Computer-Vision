...
def plotImage2Image_2dHist(MapX, MapY, thresholdX=None, thresholdY=None, logY=None, logX=None, filterX=None, filterY=None, bins=100):
    ...
    # load image files.
    img = nib.load('images/t1.nii')
    img2 = nib.load('images/t1.nii')
    ...
    x_data = img_data.reshape(
        (img_data.shape[0]*img_data.shape[1]*img_data.shape[2], -1))
    y_data = img2_data.reshape(
        (img2_data.shape[0]*img2_data.shape[1]*img2_data.shape[2], -1))
    ...
    axHisty.hist(y, bins=yedges, facecolor='blue', alpha=0.5,
                 orientation='horizontal', edgecolor='None')

    # print some correlation coefficients at the top of the image.
    mainFig.text(0.05, .95, 'r='+str(round(np.corrcoef(x, y)[1][0], 2))+'; rho='+str(
        round(spearmanr(x, y)[0], 2)), style='italic', fontsize=10)
    ...
    axHisty.set_ylabel(MapY, fontsize=10, rotation=-90,
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Plot two images against each other with a 2D histogram.')
    parser.add_argument('-x', '--MapX', help='X axis image',
                        default=None, required=True)
    parser.add_argument('-y', '--MapY', help='Y axis image',
                        default=None, required=True)
    parser.add_argument('-b', '--bins', help='Number of Histogram Bins',
                        default=100, required=False, type=int)
    parser.add_argument('-tx', '--thresholdX', help='Lower Threshold for X',
                        default=None, required=False, type=int)
    parser.add_argument('-ty', '--thresholdY', help='Lower Threshold for Y',
                        default=None, required=False, type=int)
    parser.add_argument('-fx', '--filterX', help='Exclude Number for X',
                        default=None, required=False, type=int)
    parser.add_argument('-fy', '--filterY', help='Exclude Number for Y',
                        default=None, required=False, type=int)
    parser.add_argument('-lx', '--logX', help='LogY Flag',
                        default=None, required=False, action='store_true')
    parser.add_argument('-ly', '--logY', help='LogY Flag',
                        default=None, required=False, action='store_true')
